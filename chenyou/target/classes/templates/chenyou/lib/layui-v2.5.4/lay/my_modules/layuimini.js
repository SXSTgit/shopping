layui.define(["element", "jquery"], function (exports) {
    ...
    layuimini = new function () {
        ...
        this.initMenu = function (data) {
            ...
            $.each(data, function (key, val) {
                ...
                $.each(menuList, function (index, menu) {
                    ...
                    // menu.child != undefined && menu.child != []
                ...
                sessionStorage.setItem("mpiList", JSON.stringify(mpiList));
            });
            ...
            if (urlArr.length >= 2) {
                ...
                if (!checkTab) {
                    ...
                    if (href == title) {
                        ...
                        if (menuList!=null){
                            for (var i=0;i<menuList.length;i++){
                                var item =menuList[i];
                                if (item.href!=null&&item.href.indexOf(href)!=-1){
                    ...
                    if (layuiminiHomeTab != href && layuiminiHomeHref != href) {
                        layuimini.addTab(tabId, href, title, false);
        ...
        this.cancelOrSubmit = function (targetPage, targetTitle, thisPage) {
            var mpiList = JSON.parse(sessionStorage.getItem("mpiList"));
            // var i = thisPage.split("?");
            var item = mpiList.getItem(targetPage);
            targetPage = item.key + "?" + item.value;
            // thisPage = item.key + "?" + item.value + "&id=" + i[1];
            var tab = parent.layuimini.checkTab(targetPage);
            if (!tab) {
                parent.layuimini.delTab(targetPage);
                parent.layuimini.addTab(targetPage, targetPage, targetTitle, true);
                parent.layuimini.changeTab(targetPage);
            } else {
                parent.layuimini.delTab(targetPage);
                parent.layuimini.addTab(targetPage, targetPage, targetTitle, true);
                parent.layuimini.changeTab(targetPage);
            }
            // item = mpiList.getItem(thisPage);
            parent.layuimini.delTab(thisPage);
        }
        this.goPage = function (targetPage, targetTitle) {
            var mpiList = JSON.parse(sessionStorage.getItem("mpiList"));
            var i = targetPage.split("?");
            var item = mpiList.getItem(i[0]);
            targetPage = item.key + "?" + item.value + "&" + i[1];
            var tab = parent.layuimini.checkTab(targetPage);
            if (!tab) {
                parent.layuimini.addTab(targetPage, targetPage, targetTitle, true);
                parent.layuimini.changeTab(targetPage);
    ...
    $('body').on('click', '[data-iframe-tab]', function () {
        ...
        if (!checkTab) {
            ...
            parent.layui.element.delTab(tabId);
            parent.layui.element.tabAdd('layuiminiTab', {
                title: title + '<i data-tab-close="" class="layui-icon layui-unselect layui-tab-close">ဆ</i>' //用于演示
                , content: '<iframe width="100%" height="100%" frameborder="0"  src="' + href + '"></iframe>'
                , id: tabId
